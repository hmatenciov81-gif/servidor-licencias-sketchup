  ...
  res.send(`
    ...
    <body>
      <div class="container">
        ...
        <div class="endpoint" style="border-left-color: #e53e3e;">
          <strong>GET /admin/sospechosas</strong><br>
          Panel de licencias sospechosas (requiere clave admin)
        </div>
        
        <p><a href="/admin">→ Ir al Panel de Administración</a></p>
        <p><a href="/admin/sospechosas" style="color: #e53e3e;">→ Ir al Panel de Sospechosas</a></p>
      </div>
    </body>
    </html>
  `);
    ...
    <body>
      <div class="container">
        ...
        <div class="server-info">
          ...
          <button class="refresh-button" onclick="location.href='/admin/sospechosas'" style="background:#e53e3e; margin-right: 10px;">🚨 Sospechosas</button>
...
// ============================================================================
// 🚨 PANEL VISUAL: Licencias Sospechosas
// ============================================================================
app.get('/admin/sospechosas', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      <meta charset="UTF-8">
      <title>🚨 Licencias Sospechosas</title>
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
        ...
        .nav-bar .btn-danger { background: #e53e3e; }
        .nav-bar .btn-danger:hover { background: #c53030; }
        ...
        .badge-alto { background: #fed7d7; color: #742a2a; }
        .badge-medio { background: #feebc8; color: #7c2d12; }
        .badge-bloqueada { background: #2d3748; color: #fff; }
        ...
        .btn-block { background: #e53e3e; }
        .btn-unblock { background: #38a169; }
        ...
        .detail-item .label { font-size: 11px; color: #718096; }
        .detail-item .value { font-size: 15px; font-weight: 600; color: #2d3748; }
        .device-list {
          margin-top: 15px;
        }
        .device-item {
        ...
        .device-item .dev-id { font-weight: 600; color: #2d3748; word-break: break-all; }
        .device-item .dev-info { color: #718096; margin-top: 4px; }
        .empty-state {
          text-align: center;
          padding: 60px 20px;
          color: #718096;
    ...
    <body>
      <div class="container">
        <h1>🚨 Panel de Licencias Sospechosas</h1>

        <!-- LOGIN -->
        <div id="loginSection" class="login-box">
          <h2>🔐 Acceso Administrador</h2>
          <p style="color:#718096; margin-bottom:20px;">Ingresa la clave admin para acceder al panel</p>
          <input type="password" id="adminKeyInput" placeholder="Clave de administrador" onkeydown="if(event.key==='Enter') doLogin()">
          <button onclick="doLogin()">Ingresar</button>
          <p class="login-error" id="loginError">❌ Clave incorrecta</p>
        </div>
        ...
        <div id="panelSection" style="display:none;">
          <div class="nav-bar">
            <div>
              ...
              <button class="btn-danger" onclick="cargarSospechosas()">🔄 Actualizar</button>
            </div>
            <span style="color:#718096; font-size:13px;" id="lastUpdate"></span>
          </div>

          <!-- Stats cards -->
          <div class="stats-grid" id="statsCards">
            <div class="stat-card red">
              <div class="stat-label">🚨 Sospechosas</div>
              <div class="stat-number" id="totalSospechosas">-</div>
            </div>
            <div class="stat-card orange">
              <div class="stat-label">⚠️ Nivel ALTO</div>
              <div class="stat-number" id="nivelAlto">-</div>
            </div>
            <div class="stat-card blue">
              <div class="stat-label">🔵 Nivel MEDIO</div>
              <div class="stat-number" id="nivelMedio">-</div>
            </div>
            <div class="stat-card green">
              <div class="stat-label">🔒 Bloqueadas</div>
              <div class="stat-number" id="totalBloqueadas">-</div>
            </div>
          </div>
      ...
      <script>
        ...
        function doLogin() {
          ...
          // Intentar cargar datos para verificar la clave
        ...
        function cargarSospechosas() {
          document.getElementById('tableContainer').innerHTML = '<div class="loading">🔄 Analizando licencias...</div>';

          fetch('/api/admin/sospechosas', {
            headers: { 'Authorization': 'Bearer ' + ADMIN_KEY }
          })
          .then(r => r.json())
          .then(data => {
            const lista = data.licencias || [];

            // Stats
            const alto = lista.filter(l => l.nivel === 'ALTO').length;
            const medio = lista.filter(l => l.nivel === 'MEDIO').length;
            const bloqueadas = lista.filter(l => l.bloqueada).length;

            document.getElementById('totalSospechosas').textContent = lista.length;
            document.getElementById('nivelAlto').textContent = alto;
            document.getElementById('nivelMedio').textContent = medio;
            ...
            if (lista.length === 0) {
              document.getElementById('tableContainer').innerHTML = \`
                <div class="empty-state">
                  <div class="icon">✅</div>
                  <h3>No hay licencias sospechosas</h3>
                  <p>Todas las licencias tienen un uso normal</p>
                </div>\`;
              return;
            }
            ...
            html += '<th>🔴 Nivel</th>';
            html += '<th>📊 Estado</th>';
            html += '<th>⚙️ Acciones</th>';
            html += '</tr></thead><tbody>';

            lista.forEach(lic => {
              const nivelClass = lic.nivel === 'ALTO' ? 'badge-alto' : 'badge-medio';
              const estadoBadge = lic.bloqueada
                ? '<span class="badge badge-bloqueada">🔒 Bloqueada</span>'
                : '<span class="badge badge-activa">✅ Activa</span>';

              html += '<tr>';
              html += '<td>' + lic.email + '</td>';
              html += '<td><code>' + lic.clave + '</code></td>';
              html += '<td>' + (lic.tipo || 'Premium') + '</td>';
              html += '<td style="text-align:center; font-weight:bold;">' + lic.devices_unicos + '</td>';
              html += '<td>' + lic.razon + '</td>';
              html += '<td><span class="badge ' + nivelClass + '">' + lic.nivel + '</span></td>';
              html += '<td>' + estadoBadge + '</td>';
              html += '<td>';
              html += '<button class="btn-sm btn-detail" onclick="verDetalle(\\'' + lic.clave + '\\')">🔍 Ver</button>';
              if (lic.bloqueada) {
                html += '<button class="btn-sm btn-unblock" onclick="desbloquear(\\'' + lic.clave + '\\')">🔓 Desbloquear</button>';
              } else {
                html += '<button class="btn-sm btn-block" onclick="bloquear(\\'' + lic.clave + '\\')">🔒 Bloquear</button>';
              }
              html += '</td>';
              html += '</tr>';
            });
          ...
          .catch(err => {
            document.getElementById('tableContainer').innerHTML =
              '<div class="empty-state"><div class="icon">❌</div><h3>Error al cargar datos</h3><p>' + err.message + '</p></div>';
          });
        }

        function bloquear(clave) {
          const razon = prompt('📝 Razón del bloqueo (opcional):') || 'Uso sospechoso detectado';
          if (!confirm('🔒 ¿Bloquear la licencia ' + clave + '?')) return;
          ...
          fetch('/api/admin/bloquear', {
            ...
            body: JSON.stringify({ clave, razon })
          })
          .then(r => r.json())
          .then(data => {
            if (data.ok) {
        ...
        function desbloquear(clave) {
          if (!confirm('🔓 ¿Desbloquear la licencia ' + clave + '?')) return;

          fetch('/api/admin/desbloquear', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer ' + ADMIN_KEY
        ...
        function verDetalle(clave) {
          document.getElementById('modalContent').innerHTML = '<div class="loading">🔄 Cargando detalle...</div>';
          document.getElementById('modalOverlay').classList.add('active');

          fetch('/api/admin/licencia/' + clave, {
            headers: { 'Authorization': 'Bearer ' + ADMIN_KEY }
          })
          .then(r => r.json())
          .then(data => {
            ...
            let html = '<button class="modal-close" onclick="cerrarModal()">✕</button>';
            html += '<h3>📋 Detalle de Licencia</h3>';

            html += '<div class="detail-grid">';
            html += '<div class="detail-item"><div class="label">Email</div><div class="value">' + lic.email + '</div></div>';
            html += '<div class="detail-item"><div class="label">Clave</div><div class="value" style="font-size:13px;">' + lic.clave + '</div></div>';
            html += '<div class="detail-item"><div class="label">Tipo</div><div class="value">' + (lic.tipo || 'Premium') + '</div></div>';
            html += '<div class="detail-item"><div class="label">Estado</div><div class="value">' + (lic.bloqueada ? '🔒 Bloqueada' : '✅ Activa') + '</div></div>';
            html += '<div class="detail-item"><div class="label">Devices únicos</div><div class="value">' + stats.devices_unicos + '</div></div>';
            html += '<div class="detail-item"><div class="label">Total validaciones</div><div class="value">' + stats.total_validaciones + '</div></div>';
            if (lic.fecha_expiracion) {
              html += '<div class="detail-item"><div class="label">Expiración</div><div class="value">' + new Date(lic.fecha_expiracion).toLocaleDateString('es-PE') + '</div></div>';
            }
            if (lic.razon_bloqueo) {
              html += '<div class="detail-item"><div class="label">Razón bloqueo</div><div class="value" style="color:#e53e3e;">' + lic.razon_bloqueo + '</div></div>';
            }
            html += '</div>';

            // Lista de dispositivos
            if (devices.length > 0) {
              html += '<h4 style="margin:20px 0 10px; color:#2d3748;">💻 Dispositivos detectados (' + devices.length + ')</h4>';
              html += '<div class="device-list">';
              devices.forEach((dev, i) => {
                html += '<div class="device-item">';
                html += '<div class="dev-id">📱 Device ' + (i + 1) + ': ' + (dev.device_id || 'N/A') + '</div>';
                html += '<div class="dev-info">';
                html += 'Primera vez: ' + (dev.primera_vez ? new Date(dev.primera_vez).toLocaleString('es-PE') : 'N/A');
                html += ' — Última vez: ' + (dev.ultima_vez ? new Date(dev.ultima_vez).toLocaleString('es-PE') : 'N/A');
                html += ' — Usos: ' + (dev.total_usos || 0);
                html += '</div></div>';
              });
              html += '</div>';
            }
          ...
          .catch(err => {
            document.getElementById('modalContent').innerHTML = '<p>❌ Error: ' + err.message + '</p>';
          });
        }
...
async function iniciarServidor() {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`🚨 Sospechosas: http://localhost:${PORT}/admin/sospechosas`);
