...
app.post('/api/activar', async (req, res) => {
  try {
    ...
    res.json({
      ...
      fecha_expiracion: licencia.fecha_expiracion.toISOString(),
      fecha_expiracion_legible: licencia.fecha_expiracion.toLocaleDateString('es-ES'),
      tipo_licencia: licencia.tipo,
      tipo: licencia.tipo,
      dias_restantes: Math.ceil((new Date(licencia.fecha_expiracion) - ahora) / (1000 * 60 * 60 * 24)),
...
app.post('/api/verificar', async (req, res) => {
  try {
    ...
    // Actualizar última verificación y guardar historial
    await licenciasCollection.updateOne(
      { clave: clave_licencia },
      { 
    ...
    const dias_restantes = Math.ceil((expiracion - ahora) / (1000 * 60 * 60 * 24));
    ...
    return res.json({ 
      ...
      alerta_fecha: alerta_fecha  // 🔒 NUEVO: Avisar si hay problema
    });
