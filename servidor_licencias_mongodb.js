...
// ============================================================================
// IMPORTANTE: Este middleware protege TODOS los endpoints /api/*
// excepto los públicos definidos en rutasPublicas
// ============================================================================
...
app.use('/api', (req, res, next) => {
  ...
  const rutasPublicas = [
    ...
    '/api/instalador/download'    // Descarga del instalador
  ];
  
  // Si la ruta está en la lista pública, permitir sin API Key
  if (rutasPublicas.includes(req.path)) {
    return next();
  }
  
  // Validar que el header X-API-Key esté presente
  const apiKey = req.headers['x-api-key'];
  
  if (!apiKey) {
    console.log('⚠️ [SEGURIDAD] Petición SIN API Key rechazada');
...
// ============================================================================
// 🛡️ ENDPOINT DE REPORTES DE SEGURIDAD (Opcional)
// ============================================================================
// Este endpoint recibe reportes cuando se detecta manipulación del código
// ============================================================================

app.post('/api/reporte', async (req, res) => {
  try {
    const { evento, timestamp } = req.body;
    
    console.log('⚠️⚠️⚠️ [ALERTA DE SEGURIDAD] ⚠️⚠️⚠️');
    console.log(`  Evento: ${evento}`);
    console.log(`  Fecha: ${new Date(timestamp * 1000).toLocaleString('es-PE')}`);
    console.log(`  IP: ${req.ip}`);
    console.log(`  User-Agent: ${req.headers['user-agent']}`);
    
    // Opcional: Guardar en base de datos para análisis posterior
    if (db) {
      await db.collection('reportes_seguridad').insertOne({
        evento,
  ...
  res.send(`
    ...
    <body>
      <div class="container">
        ...
        <div class="security">
          <strong>🔒 Seguridad Activa:</strong> Todos los endpoints requieren API Key válida (X-API-Key header)
        </div>
        ...
        <div class="endpoint">
          ...
          Generar nueva licencia (requiere clave admin + API Key)
        </div>
        
        <div class="endpoint">
          <strong>POST /api/activar</strong><br>
          Activar licencia con email y clave (requiere API Key)
        </div>
        
        <div class="endpoint">
          <strong>POST /api/verificar</strong><br>
          Verificar estado de licencia (requiere API Key)
        </div>
        
        <div class="endpoint">
          <strong>POST /api/validar</strong><br>
          Validar licencia existente (requiere API Key)
        </div>
        
        <div class="endpoint">
          <strong>POST /api/cambiar-dispositivo</strong><br>
          Liberar dispositivo para cambio de PC (requiere clave admin + API Key)
        </div>
        
        <div class="endpoint">
          <strong>POST /api/reporte</strong><br>
          Recibir reportes de seguridad (requiere API Key)
        </div>
        
        <div class="endpoint">
          <strong>GET /admin</strong><br>
          Panel de administración (sin API Key - uso manual)
        </div>
